...
window.onload = function init() {
    ...
    canvas.addEventListener("mousedown", function(event){
        if (event.which == 1){
            var curPoint = vec2(Math.round(event.offsetX/canvas.clientWidth*CanvasResolution.width), Math.round((1 - event.offsetY/canvas.clientHeight)*CanvasResolution.height));
            if (drawType == "line"){
                isDraging = true;
                ...
                if (isDrawingCurve == 0){ // 第一次点击
                    isDrawingCurve = 1;
                    shapes.push({type: drawType, p1: curPoint, p2: null, p3: null, p4: curPoint, isComplete: false});
                }
    ...
    canvas.addEventListener("mousemove", function(event){
        ...
        if (drawType == "line"){
            ...
            if (!isDraging) return;
            shapes[shapes.length - 1].radius = length(subtract(shapes[shapes.length - 1].center, curPoint));
        }
        else if (drawType == "polygon"){
    ...
    window.onresize = function(){
        ...
        canvas.clientHeight = canvas.clientWidth / CanvasResolution.width * CanvasResolution.height;
...
function render(){
    ...
    for(shape of shapes){
        switch(shape.type){
            ...
            case "curve":{
                if (shape.p2 == null && shape.p3 == null){ // 只有两个点时，绘制直线
                    drawLine(shape.p1, shape.p4);
                    break;
                }
                else if(shape.p3 == null){ // 有三个点时，假设最后一个控制点与第一个控制点对称
...
function setDrawType(type){
    ...
    switch(type){
        ...
        case "polygon":{
            tipElement.innerText = "点击鼠标左键来添加多边形的顶点，双击结束多边形编辑";
            break;
        }
        case "curve":{
            tipElement.innerText = "三次贝塞尔曲线，点击鼠标左键来依次添加四个控制点，前两个点为起始和终止点(p1, p4)，后两个点为剩余两个控制点(p2, p3)";
            break;
        }
    }
