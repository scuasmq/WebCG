...
先生成r=sqrt(Pi*x*x+Pi*y*y),再用函数f(r)=sin(r)/(r)生成data数组
X的范围是（-2，2），Z的范围是（-2,2）,Y取用data对应位置的值*/
var data = [];
...
window.onload = function init()
{
	...
	//开启“多边形偏移模式”：使多边形稍微原理观察者，这样观察者就能看到想要显示的线段了。并设置多边形偏移参数。
    ...
    document.getElementById("Button2").onclick = function(){near  *= 0.9; far *= 0.9;};//平行视见体范围调整
...
function render()
{
    ...
    for(var i=0; i<pointsArray.length; i+=4) {
        ...
        gl.drawArrays( gl.TRIANGLE_FAN, i, 4 ); //按四边形（两个红色三角扇）图元显示
        gl.uniform4fv(fColor, flatten(black));
        gl.drawArrays( gl.LINE_LOOP, i, 4 );//按线带（四边形）图元显示
    }
